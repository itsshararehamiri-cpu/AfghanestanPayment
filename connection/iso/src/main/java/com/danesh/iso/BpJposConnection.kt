...
class BpJposConnection @Inject constructor(
    ...
    private val packager: ISOPackager = BpIso93BPackager()
    ...
    override suspend fun request(message: IsoMessage, isEcho: Boolean): IsoMessage? {
        ...
        response.print("resp>>")
        channel.lastReceivedIsoBody?.let { raw -> response.requireBp().setRawPackedBody(raw) }
        return response
    }
    ...
    override suspend fun receive(): IsoMessage? {
        return try {
         ...
         val temp=   messageProvider.create().also {
                ...
                channel.lastReceivedIsoBody?.let { raw -> it.requireBp().setRawPackedBody(raw) }
            }
            temp.print("resp>>")
            temp
        } catch (e: Exception) {
