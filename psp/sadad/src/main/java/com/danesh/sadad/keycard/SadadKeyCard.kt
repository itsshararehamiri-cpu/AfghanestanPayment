 ...
 * کاری رمزشده با همان جفت RSA استفاده می‌شوند. ایندکس استفاده‌شده در هر دو مرحله باید یکسان
 * باشد: کلید ایندکس n از کارت B/C با کلید RSA همان ایندکس n از کارت A رمزگشایی می‌شود.
 *
 * تفاوت کارت B و C در محتوای همان ۶ کلید مستند در [SadadKeyNumber] است، نه در ساختار دستور:
 * روی کارت B هر ۶ کلید یک مقدار یکسان دارند (کارت B صرفاً برای تحویل یک کلید واحد به‌عنوان
 * هر ۶ نقش استفاده می‌شود)، در حالی که روی کارت C این ۶ کلید مقادیر متمایز از هم دارند
 * (برای راه‌اندازی اولیه). منطق خواندن/رمزگشایی/ذخیره برای هر دو کارت یکسان است؛ فقط باید
 * توجه داشت مقایسه یا فرض یکسان‌بودن این ۶ مقدار فقط برای کارت B معتبر است.
 */
