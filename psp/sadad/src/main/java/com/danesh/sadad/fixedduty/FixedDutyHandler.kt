...
@Singleton
class FixedDutyHandler @Inject constructor(
    private val messageBuilder: SadadFixedDutyMessageBuilder,
    private val messages: SadadTransactionMessages,
    private val transport: SadadIsoHandlerSupport,
) : HandlerTransaction<SadadFixedDutyRequest, SadadNetworkResult, IsoMessage>() {

    override val isReversible: Boolean = false
    override val needReport: Boolean = false

    override fun buildMessage(request: SadadFixedDutyRequest): IsoMessage =
        messageBuilder.build(request)

    override fun queueFailure(request: SadadFixedDutyRequest): SadadNetworkResult {
