package com.danesh.sadad.mac

import javax.crypto.Cipher
import javax.crypto.spec.SecretKeySpec

/**
 * ANSI X9.19 (Retail MAC) با padding صفر طبق ISO 9797-1 روش ۱ — همان چیزی که هاست سداد حساب می‌کند.
 *
 * نکته مهم: اگر طول داده مضرب ۸ باشد هیچ بلوک padding اضافه نمی‌شود.
 * PED دستگاه K9 با TYPE_X919_00 در این حالت یک بلوک ۸ بایتی صفر اضافه می‌کند و MAC
 * پیام‌هایی مثل پرداخت قبض (طول ۱۵۲) اشتباه درمی‌آید (کد پاسخ 19).
 */
object SadadAnsiX919Mac {

    const val MAC_LENGTH = 8

    fun calculate(key: ByteArray, data: ByteArray): ByteArray {
        require(key.size == 16 || key.size == 24) {
            "X9.19 key must be 16 or 24 bytes, was ${key.size}"
        }
        val k1 = key.copyOfRange(0, 8)
        val k2 = key.copyOfRange(8, 16)
        val k3 = if (key.size == 24) key.copyOfRange(16, 24) else k1
        val padded = zeroPad(data)
        var state = ByteArray(8)
        var offset = 0
        while (offset < padded.size) {
            val block = padded.copyOfRange(offset, offset + 8)
            state = desEcb(k1, xor8(state, block), encrypt = true)
            offset += 8
        }
        state = desEcb(k2, state, encrypt = false)
        return desEcb(k3, state, encrypt = true)
    }

    /** ISO 9797-1 روش ۱: فقط تا مرز ۸ بایت صفر اضافه می‌شود؛ داده هم‌تراز دست نمی‌خورد. */
    internal fun zeroPad(data: ByteArray): ByteArray {
        val mod = data.size % 8
        if (data.isNotEmpty() && mod == 0) return data
        return data.copyOf(data.size + (8 - mod))
    }

    private fun xor8(left: ByteArray, right: ByteArray): ByteArray =
        ByteArray(8) { index -> (left[index].toInt() xor right[index].toInt()).toByte() }

    private fun desEcb(key8: ByteArray, block: ByteArray, encrypt: Boolean): ByteArray {
        val cipher = Cipher.getInstance("DES/ECB/NoPadding")
        cipher.init(
            if (encrypt) Cipher.ENCRYPT_MODE else Cipher.DECRYPT_MODE,
            SecretKeySpec(key8, "DES"),
        )
        return cipher.doFinal(block)
    }
}
