...
@Singleton
class BpCashOutMessageBuilder @Inject constructor(
    ...
    suspend fun build(request: CashOutUserInput): IsoMessage {
        val config = contextProvider.getTerminalConfig()
        val message = messageProvider.create().apply {
            mti = BpKeyConfig.CASH_OUT_MTI
            processingCode = BpKeyConfig.CASH_OUT_PROCESSING_CODE
            ...
            messageReasonCode = BpKeyConfig.CASH_OUT_MESSAGE_REASON
            track2 = request.track2
            terminalId = config.terminalId
            currency = BpKeyConfig.CASH_OUT_CURRENCY
            nii = BpKeyConfig.CASH_OUT_NII
