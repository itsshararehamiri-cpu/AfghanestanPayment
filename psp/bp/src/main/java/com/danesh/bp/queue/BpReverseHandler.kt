...
@Singleton
class BpReverseHandler @Inject constructor(
    private val reverseMessageBuilder: BpReverseMessageBuilder,
    private val sessionClock: TransactionSessionClock,
) : HandlerTransaction<BpReverseRequest, BpAdviceResult, IsoMessage>() {
    ...
    override fun buildMessage(request: BpReverseRequest): IsoMessage {
        val item = request.queueItem
        sessionClock.capture(TransactionClock(date = item.date, time = item.time))
        return kotlinx.coroutines.runBlocking {
            reverseMessageBuilder.build(item)
        }
    }

    override fun queueFailure(request: BpReverseRequest): BpAdviceResult {
